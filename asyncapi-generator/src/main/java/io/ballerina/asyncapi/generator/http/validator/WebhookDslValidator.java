...
public final class WebhookDslValidator {
    ...
    private static void validatePayloadInput(String input, List<String> extractedVariables)
        ...
        String withoutFuncCalls = headerMatcher.replaceAll(" ");
        Matcher configMatcher = CONFIG_FUNC_PATTERN.matcher(withoutFuncCalls);
        withoutFuncCalls = configMatcher.replaceAll(" ");

        Set<String> allowedCustomVariables = new HashSet<>(extractedVariables);

        Matcher tokenMatcher = DOLLAR_TOKEN_PATTERN.matcher(withoutFuncCalls);
